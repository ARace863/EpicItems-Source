...
public class MainCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender.hasPermission("epicitems.main") || sender.hasPermission("epicitems.admin")){
            if (label.equalsIgnoreCase("epicitems")) { // Main command
                ...
                if (args.length >= 1) {
                    ...
                    sender.sendMessage(ChatColor.AQUA + "[EpicItems] " + ChatColor.RED + "/epicitems <info, help, giveItem, items, stats, checkForEpicItem, checkForUpdates>");
                }
            }
        }else{
