        ...
        if (sender.hasPermission("epicitems.main") || sender.hasPermission("op")){
            if (label.equalsIgnoreCase("epicitems")) { // Main command
                if (args.length >= 1) {
                        ...
                        try {
                            ...
                            if (args[1].equalsIgnoreCase("warrioraxe")) {
                                ...
                                if (target == null) {
                                    ...
                                    target.sendMessage(ChatColor.GREEN + "You have recived the " + ChatColor.DARK_RED + "Warrior Axe");
                                    if (Bukkit.getPluginManager().isPluginEnabled("NoteBlockAPI")) {
                                        if (sender instanceof Player) {
                                            Song jingle = NBSDecoder.parse(new File("C:\\Users\\USER\\Desktop\\Development\\Spigot Plugins\\EpicItems\\src\\main\\java\\me\\arace863\\epicitems\\Songs\\unlock.nbs"));
                                            RadioSongPlayer rsp = new RadioSongPlayer(jingle);
                            ...
                            if (args[1].equalsIgnoreCase("fireballrod")) {
                                ...
                                if (target == null) {
                                    ...
                                    target.sendMessage(ChatColor.GREEN + "You have recived the " + ChatColor.DARK_RED + "Fireball Rod");
                                    if (Bukkit.getPluginManager().isPluginEnabled("NoteBlockAPI")) {
                                        if (sender instanceof Player) {
                                            Song jingle = NBSDecoder.parse(new File("C:\\Users\\USER\\Desktop\\Development\\Spigot Plugins\\EpicItems\\src\\main\\java\\me\\arace863\\epicitems\\Songs\\unlock.nbs"));
                                            RadioSongPlayer rsp = new RadioSongPlayer(jingle);
                    ...
                    if (args[0].equalsIgnoreCase("generateCustomMap")) {
                        if (sender instanceof Player) {
                            Player player = (Player) sender;
                            ...
                            MapUtils renderer = new MapUtils();
                            if (!renderer.load(args[1])) {
                                utilOptions.playerMessage(player, "&cCould not load map, maybe your link isn't working");
                                return true;
                            }
                            ...
                            player.getInventory().addItem(map);
                            utilOptions.playerMessage(player, "&aLoaded map!");
                        }
                    }
                    ...
                    sender.sendMessage(ChatColor.AQUA + "[EpicItems] " + ChatColor.RED + "/epicitems <info, help, giveItem, items, stats, checkForEpicItem, checkForUpdates, itemStore, setCooldown, generateCustomMap>");
                }
            }
        }else{
